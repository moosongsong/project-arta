...
class PieceList(ListView):
    model = Piece
    paginate_by = 8
    template_name = 'exhibition/ARTA_user_exhibition_show_page.html'
...
class ExhibitionListForArtist(ListView):
    ...
    def get_queryset(self):
        ...
        if open_mode == 'all':
            ...
            exhibition_list = Exhibition.objects.filter(user=self.request.user, end_at__gte=datetime.datetime.now(), start_at__lte=datetime.datetime.now()).order_by('end_at')
        elif open_mode == 'close':
            exhibition_list = Exhibition.objects.filter(user=self.request.user, end_at__lt=datetime.datetime.now()).order_by('end_at')
        elif open_mode == 'ready':
            exhibition_list = Exhibition.objects.filter(user=self.request.user, start_at__gt=datetime.datetime.now()).order_by('end_at')
        return exhibition_list
